// --- COLOR (overbrightening on top of the theme's cursor-color) ---
const float SATURATION    = 6.0;  // 1.0 = theme color unchanged; >1.0 punches chroma
const float GLOW_STRENGTH = 0.03;  // additive halo outside the trail; 0.0 disabled, 0.4–1.0 = blaze-style spill
const float GLOW_RADIUS   = 0.08; // halo size in normalized (-1..1) coords
const float GLOW_FADE_DURATION = 0.5; // extra seconds after DURATION where the glow eases to zero

float _lum = dot(iCurrentCursorColor.rgb, vec3(0.2126, 0.7152, 0.0722));
vec4 TRAIL_COLOR = vec4(clamp(mix(vec3(_lum), iCurrentCursorColor.rgb, SATURATION), 0.0, 1.0), iCurrentCursorColor.a);
...
const float TRAIL_THICKNESS_Y = 1.0; // 1.0 = full cursor height, 0.0 = zero height, >1.0 = funky aah
const float TRAIL_THICKNESS_X = 0.8; // 1.0 = full cursor width at endpoints (block caps), 0.0 = needle (pointed caps)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    if (lineLength > minDist && baseProgress < DURATION + GLOW_FADE_DURATION) {
        // Body visible during the smear, glow lingers past it with an eased fade.
        float bodyAlive = 1.0 - step(DURATION, baseProgress);
        float glowFade = 1.0 - smoothstep(DURATION, DURATION + GLOW_FADE_DURATION, baseProgress);
        // defining corners of cursors

        // Y (Height) with TRAIL_THICKNESS_Y
        float cc_half_height = currentCursor.w * 0.5;
        float cc_center_y = currentCursor.y - cc_half_height;
        float cc_new_half_height = cc_half_height * TRAIL_THICKNESS_Y;
        float cc_new_top_y = cc_center_y + cc_new_half_height;
        float cc_new_bottom_y = cc_center_y - cc_new_half_height;

        // X (Width) with TRAIL_THICKNESS_X
        ...
        float cp_new_half_height = cp_half_height * TRAIL_THICKNESS_Y;
        ...
        float finalAlpha = trail.a * shapeAlpha * bodyAlive;

        // newColor.a to preserve the background alpha.
        newColor = mix(newColor, vec4(trail.rgb, newColor.a), finalAlpha);

        // additive bloom halo outside the trail body, with time-based fade after DURATION
        if (GLOW_STRENGTH > 0.0 && sdfTrail > 0.0 && glowFade > 0.0) {
            float glow = 1.0 - smoothstep(0.0, GLOW_RADIUS, sdfTrail);
            glow *= glow; // squared falloff for HDR-ish spill
            newColor.rgb = min(newColor.rgb + trail.rgb * glow * GLOW_STRENGTH * trail.a * glowFade, vec3(1.0));
        }
