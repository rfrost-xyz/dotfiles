...
export default function sortSlashCommands(pi: ExtensionAPI) {
	if (!isEnabled("autocomplete")) return;
		...
		ctx.ui.addAutocompleteProvider((current) => ({
			async getSuggestions(lines, cursorLine, cursorCol, options) {
				...
				if (!suggestions?.items || !isSlashCommandCompletion(lines, cursorLine, cursorCol, suggestions)) {
					// biome-ignore lint/suspicious/noExplicitAny: pi's autocomplete return type allows undefined items but TS narrows it away here
